<template>
  <div>
    <div v-if="userStore.isAuthenticated" class="bg-white rounded-lg shadow-md p-6">
      ...
      <button @click="handleLogout" class="w-full bg-red-500 text-white py-2 rounded hover:bg-red-600 transition duration-300">Logout</button>
      <button @click="downloadLibrary" class="w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600 transition duration-300 mt-4">Download Library</button>
    </div>
    <div v-else class="bg-white rounded-lg shadow-md p-6">
      ...
      <button @click="handleShowSignUp" class="w-full mt-4 bg-green-500 text-white py-2 rounded hover:bg-green-600 transition duration-300">Sign Up</button>
      <p v-if="loginError" class="mt-4 text-red-600 bg-red-100 border border-red-400 rounded p-2">{{ loginError }}</p>
    </div>
  </div>
</template>
...
onMounted(() => {
  // Optionally, you can prefetch the download URL here
})
</script>
